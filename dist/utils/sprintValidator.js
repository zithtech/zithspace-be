...
exports.SprintValidator = void 0;
...
class SprintValidator {
     ...
     */
    static async hasActiveSprint(projectId, tenantId) {
     ...
     */
    static async getActiveSprint(projectId, tenantId) {
        const activeSprint = await database_1.prisma.releasePlan.findFirst({
            where: {
                projectId,
                tenantId,
     ...
     */
    static async validateNewSprint(projectId, tenantId) {
        const activeSprint = await database_1.prisma.releasePlan.findFirst({
            where: {
                projectId,
                tenantId,
        ...
        if (activeSprint) {
            return {
                valid: false,
                error: "Cannot create a new sprint while another sprint is active. Please complete the current sprint first.",
                activeSprint,
            };
        }
        return { valid: true };
     ...
     */
    static async validateStartSprint(sprintId, projectId, tenantId) {
        ...
        if (hasActive) {
            return {
                valid: false,
                error: "Another sprint is already active in this project. Complete it before starting a new sprint.",
            };
        }
     ...
     */
    static async validateCompleteSprint(sprintId, tenantId) {
        const sprint = await database_1.prisma.releasePlan.findFirst({
            where: { id: sprintId, tenantId },
        });
        if (!sprint) {
     ...
     */
    static async calculateSprintMetrics(sprintId, tenantId) {
        const sprint = await database_1.prisma.releasePlan.findFirst({
            where: { id: sprintId, tenantId },
            include: {
                tickets: {
        ...
        const completedTickets = sprint.tickets.filter((t) => t.status === "completed").length;
        const inProgressTickets = sprint.tickets.filter((t) => t.status === "in_progress").length;
        const notStartedTickets = sprint.tickets.filter((t) => t.status === "not_started" || t.status === "open").length;
        const totalPoints = sprint.tickets.reduce((sum, t) => sum + (t.storyPoint || 0), 0);
     ...
     */
    static async getProjectVelocity(projectId, tenantId, lastNSprints = 5) {
        const completedSprints = await database_1.prisma.releasePlan.findMany({
            where: {
                projectId,
                tenantId,
                type: "sprint_plan",
        ...
        const totalCompletedPoints = completedSprints.reduce((sum, sprint) => sum + (sprint.completedPoints || 0), 0);
...
exports.SprintValidator = SprintValidator;
//# sourceMappingURL=sprintValidator.js.map