...
exports.ReimbursementAttachmentController = exports.ReimbursementApprovalController = exports.ReimbursementItemController = exports.ReimbursementRequestController = void 0;
...
class ReimbursementCategoryController {
    ...
    static async uploadFile(req, res) {
        try {
            ...
            const { fileUrl, fileSize, fileType } = await uploadFileToR2(dataUri, file.originalname, req.tenantId);
        ...
        try {
            ...
            await tenantAwarePrisma.withTenant(tenantId, async (client) => {
                ...
                const processedItems = await Promise.all(items.map(async (item) => {
                    ...
                    const validAttachments = processedAttachments.filter((a) => a && a.url);
                    return {
                        ...item,
                ...
                try {
                    ...
                    processedItems.forEach((processedItem, index) => {
                        ...
                        if (createdItem && processedItem.processedAttachments) {
                            processedItem.processedAttachments.forEach((att) => {
                                attachmentPromises.push(client.reimbursementAttachment.create({
                                    data: {
                                        tenantId,
                ...
                const updated = await client.$transaction(async (tx) => {
                    try {
                        ...
                        processedItems.forEach((processedItem, index) => {
                            ...
                            if (createdItem && processedItem.processedAttachments) {
                                processedItem.processedAttachments.forEach((att) => {
                                    attachmentPromises.push(tx.reimbursementAttachment.create({
                                        data: {
                                            tenantId,
                                            fileName: att.name || 'attachment',
                                            fileUrl: att.url,
