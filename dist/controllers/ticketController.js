...
const novuService_1 = require("@/services/novuService");
class TicketController {
    /**
     * Helper to notify project members via Novu
     */
    static async notifyProjectMembers(tenantId, projectId, workflowId, payload, excludeUserId) {
        try {
            ...
            await novuService_1.novuService.triggerNotification(workflowId, subscriberIds, payload);
        }
        catch (error) {
    ...
    static async createTicket(req, res) {
        try {
            ...
            TicketController.notifyProjectMembers(req.tenantId, projectId, "ticket-created", {
                ticketId: ticket.id,
        ...
        try {
            ...
            try {
                ...
                if (hasMeaningfulUpdate) {
                    TicketController.notifyProjectMembers(req.tenantId, existingTicket.projectId, "ticket-updated", {
                        ticketId: ticket.id,
                        ticketTitle: ticket.title,
                        actorName: req.user.name,
    ...
    static async addComment(req, res) {
        try {
            ...
            // Notify project members
            TicketController.notifyProjectMembers(req.tenantId, ticket.projectId, "ticket-updated", // User said "A ticket is updated (... or new comment)". Often comments have their own trigger like 'ticket-commented' but user grouped them. I will use 'ticket-commented' for clarity or 'ticket-updated' if strict? 
