...
class LeaveController {
    ...
    async applyLeave(req, res) {
        try {
            ...
            const { type, startDate, endDate, duration, durationType, reason, attachments, } = req.body;
            ...
            const leave = await database_1.prisma.leave.create({
                data: {
                    tenantId,
                    userId,
                    type,
    ...
    async getMyLeaves(req, res) {
        try {
            ...
            const [leaves, total] = await Promise.all([
                ...
                database_1.prisma.leave.count({ where }),
            ]);
            res.status(200).json({
                success: true,
                data: leaves,
    ...
    async getLeaveById(req, res) {
        try {
            ...
            const userId = req.user.id;
            const userRole = req.user.role;
            const leave = await database_1.prisma.leave.findFirst({
                where: { id, tenantId },
                include: {
                    user: {
                        select: {
            ...
            const canAccess = leave.userId === userId ||
    ...
    async approveLeave(req, res) {
        try {
            ...
            const canApprove = userRole === "super_admin" || leave.user.reportsToId === approverId;
            if (!canApprove) {
                res.status(403).json({
                    success: false,
            ...
            // Update leave status
            const updatedLeave = await database_1.prisma.leave.update({
                where: { id },
                data: {
                    status: "approved",
    ...
    async rejectLeave(req, res) {
        try {
            ...
            const canReject = userRole === "super_admin" || leave.user.reportsToId === approverId;
            if (!canReject) {
                res.status(403).json({
                    success: false,
