...
class TenantController {
    ...
    static async resolve(req, res) {
        try {
         ...
         LIMIT 1`, [requested]);
            ...
            if (!tenant) {
                const alias = await dbpool_1.default.query(`SELECT t.id, t.name, t.subdomain, t.plan_type, t.is_active, t.is_setup_complete
    ...
    static async completeSetup(req, res) {
        try {
            ...
            const conflict = await dbpool_1.default.query(`SELECT 1 FROM tenants WHERE subdomain = $1 AND id != $2
          UNION ALL
         SELECT 1 FROM tenant_subdomain_aliases WHERE subdomain = $1 AND tenant_id != $2
         LIMIT 1`, [newSubdomain, req.tenantId]);
            ...
            try {
                await client.query("BEGIN");
                const previous = await client.query("SELECT subdomain FROM tenants WHERE id = $1 FOR UPDATE", [req.tenantId]);
                const oldSubdomain = previous.rows[0]?.subdomain;
                updated = await client.query(`UPDATE tenants
           ...
           RETURNING id, name, subdomain`, [workspaceName.trim(), newSubdomain, req.tenantId]);
                if (oldSubdomain && oldSubdomain !== newSubdomain) {
                    await client.query(`INSERT INTO tenant_subdomain_aliases (subdomain, tenant_id)
             VALUES ($1, $2)
             ON CONFLICT (subdomain) DO NOTHING`, [oldSubdomain, req.tenantId]);
                }
                ...
                await client.query("DELETE FROM tenant_subdomain_aliases WHERE subdomain = $1", [newSubdomain]);
                await client.query("COMMIT");
            }
            catch (err) {
                await client.query("ROLLBACK");
            ...
            const tokens = jwt_1.JWTUtils.generateTokenPair(req.user);
            res.status(200).json({
                success: true,
                data: {
