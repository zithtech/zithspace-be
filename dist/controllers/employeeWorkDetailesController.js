...
class EmployeeWorkDetailController {
    ...
    static async createWorkDetail(req, res) {
        try {
            ...
            await (0, onboardingPool_1.withTenant)(req.tenantId, async (db) => {
                // check employee exists
                const employeeRes = await db.query(`SELECT * FROM employees WHERE id = $1 LIMIT 1`, [employeeId]);
                ...
                const workDetailRes = await db.query(`INSERT INTO employee_work_details
    ...
    static async getWorkDetailByEmployee(req, res) {
        try {
            ...
            await (0, onboardingPool_1.withTenant)(req.tenantId, async (db) => {
                ...
                const userRes = await db.query(`SELECT id, name, employee_id, reports_to_id, position_id
             FROM users
            WHERE id = $1
            LIMIT 1`, [inputId]);
                ...
                let position = null;
                if (user?.position_id) {
                    const posRes = await db.query(`SELECT p.*, d.id AS d_id, d.name AS d_name, d.code AS d_code,
              ...
              LIMIT 1`, [user.position_id]);
                ...
                if (user?.reports_to_id) {
                    const reportsToRes = await db.query(`SELECT name FROM users WHERE id = $1 LIMIT 1`, [user.reports_to_id]);
                ...
                const workDetailRes = await db.query(`SELECT * FROM employee_work_details WHERE employee_id = $1 LIMIT 1`, [employeeId]);
                ...
                if (!position && workDetailRow?.position_id) {
                    const wdPosRes = await db.query(`SELECT p.*, d.id AS d_id, d.name AS d_name, d.code AS d_code,
                    d.tenant_id AS d_tenant_id, d.employment_type AS d_employment_type,
                    d.description AS d_description, d.head_id AS d_head_id,
                    d.is_active AS d_is_active, d.created_by_id AS d_created_by_id,
                    d.updated_by_id AS d_updated_by_id, d.created_at AS d_created_at,
               ...
               LEFT JOIN departments d ON d.id = p.department_id
              WHERE p.id = $1
              LIMIT 1`, [workDetailRow.position_id]);
                ...
                if (!reportingManagerName) {
                    const projectMappingRes = await db.query(`SELECT * FROM employee_project_mappings
              WHERE employee_id = $1
              ...
              LIMIT 1`, [employeeId]);
                    ...
                    if (projectMapping?.reporting_manager) {
                        ...
                        if (isUuid) {
                            ...
                            const userManagerRes = await db.query(`SELECT name FROM users WHERE id = $1 LIMIT 1`, [projectMapping.reporting_manager]);
                            ...
                            if (userManager) {
                                ...
                                const empManagerRes = await db.query(`SELECT first_name, last_name FROM employees WHERE id = $1 LIMIT 1`, [projectMapping.reporting_manager]);
                ...
                const noticePolicyRes = await db.query(`SELECT * FROM exit_notice_policies
            WHERE tenant_id = $1
              AND (
                (level_type = 'Positions' AND level_id = $2)
                OR (level_type = 'Grades' AND level_id = $3)
              )
            LIMIT 1`, [tenantId, position?.id || "", position?.gradeId || ""]);
    ...
    static async getWorkDetailById(req, res) {
        try {
            const { id } = req.params;
            await (0, onboardingPool_1.withTenant)(req.tenantId, async (db) => {
                const workDetailRes = await db.query(`SELECT * FROM employee_work_details WHERE id = $1 LIMIT 1`, [id]);
                const workDetail = mapWorkDetail(workDetailRes.rows[0]);
                if (!workDetail) {
                    res.status(404).json({
                        success: false,
    ...
    static async updateWorkDetail(req, res) {
        try {
            ...
            const { id } = req.params;
            await (0, onboardingPool_1.withTenant)(req.tenantId, async (db) => {
                const existingRes = await db.query(`SELECT * FROM employee_work_details WHERE id = $1 LIMIT 1`, [id]);
                ...
                const updatedRes = await db.query(`UPDATE employee_work_details
              SET position_id = $1,
                  team = $2,
                  employee_type = $3,
                  work_location = $4,
    ...
    static async deleteWorkDetail(req, res) {
        try {
            const { id } = req.params;
            await (0, onboardingPool_1.withTenant)(req.tenantId, async (db) => {
                const existingRes = await db.query(`SELECT id FROM employee_work_details WHERE id = $1 LIMIT 1`, [id]);
                const existing = existingRes.rows[0];
                if (!existing) {
                    res.status(404).json({
                        success: false,
                ...
                await db.query(`DELETE FROM employee_work_details WHERE id = $1`, [id]);
                res.status(200).json({
                    success: true,
