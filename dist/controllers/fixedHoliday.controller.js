...
class FixedHolidayController {
    static async createFixedHoliday(req, res) {
        try {
            ...
            res.status(201).json({ success: true, data: fixedHoliday, message: "Holiday created successfully" });
        }
        catch (error) {
            console.error("Error creating fixed holiday:", error);
            res.status(500).json({ success: false, error: "Failed to create fixed holiday" });
        }
    }
    static async getFixedHolidays(req, res) {
        try {
            ...
            res.status(500).json({ success: false, error: "Failed to fetch fixed holidays" });
        }
    }
    static async getFixedHolidayById(req, res) {
        try {
            ...
            if (!holiday) {
                res.status(404).json({ success: false, error: "Fixed holiday not found" });
                return;
            }
            ...
            res.status(500).json({ success: false, error: "Failed to fetch fixed holiday" });
        }
    }
    static async updateFixedHoliday(req, res) {
        try {
            ...
            res.status(200).json({ success: true, data: updatedHoliday, message: "Holiday updated successfully" });
        }
        catch (error) {
            console.error("Error updating fixed holiday:", error);
            res.status(500).json({ success: false, error: "Failed to update fixed holiday" });
        }
    }
    static async deleteFixedHoliday(req, res) {
        try {
            ...
            res.status(200).json({ success: true, message: "Fixed holiday deleted successfully" });
        }
        catch (error) {
            console.error("Error deleting fixed holiday:", error);
            res.status(500).json({ success: false, error: "Failed to delete fixed holiday" });
        }
    }
}
