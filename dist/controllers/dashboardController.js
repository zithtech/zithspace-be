    ...
    static async getDashboardSummary(req, res) {
        try {
            ...
            const [totalMembers, activeProjectsCount, allTickets, monthlyRevenue, recentActivities, usersUpcomingTasks, activeProjects, todayAttendance, pendingLeaveApprovals, userLeaveStats, todayLeaves, dailyUpdatesStats, topPerformer, lateAndOvertime, upcomingBirthdays,] = await Promise.all([
                ...
                database_1.prisma.ticketActivityLog.findMany({
                    where: {
                        tenantId: req.tenantId,
                    },
                    include: {
                ...
                (async () => {
                    ...
                    const activeUsers = await database_1.prisma.user.findMany({
                        where: {
                            tenantId: req.tenantId,
                            isActive: true,
                        },
                    ...
                    const findRecord = (userId) => attendanceRecords.find((r) => r.userId === userId);
                    ...
                    return {
                        ...
                        attendanceRate: activeUsers.length > 0
                ...
                (async () => {
                    ...
                    const count = await database_1.prisma.leave.count({ where });
                    return count;
                })(),
                // 10. User Leave Statistics (current month)
                (async () => {
                    const userId = req.user.id;
                    const leaves = await database_1.prisma.leave.findMany({
                        where: {
                            tenantId: req.tenantId,
                            userId,
                            startDate: {
                ...
                database_1.prisma.leave.findMany({
                    where: {
                        tenantId: req.tenantId,
                        status: "approved",
                ...
                (async () => {
                    const submitted = await database_1.prisma.statusUpdate.count({
                        where: {
                            tenantId: req.tenantId,
                            date: {
                                gte: startOfToday,
                    ...
                    const totalActiveMembers = await database_1.prisma.user.count({
                        where: {
                            tenantId: req.tenantId,
                            isActive: true,
                        },
                    });
                    const avgHours = await database_1.prisma.statusUpdate.aggregate({
                        where: {
                            tenantId: req.tenantId,
                            date: {
                                gte: startOfToday,
                ...
                (async () => {
                    const completedTickets = await database_1.prisma.ticket.groupBy({
                        by: ["assigneeId"],
                        where: {
                            tenantId: req.tenantId,
                    ...
                    if (completedTickets.length > 0 && completedTickets[0].assigneeId) {
                        const topUser = await database_1.prisma.user.findUnique({
                ...
                (async () => {
                    const lateCount = await database_1.prisma.attendance.count({
                        where: {
                            tenantId: req.tenantId,
                            date: { gte: startOfToday, lte: endOfToday },
                            lateMinutes: { gt: 0 },
                    ...
                    const overtimeCount = await database_1.prisma.attendance.count({
                        where: {
                            tenantId: req.tenantId,
                            date: { gte: startOfToday, lte: endOfToday },
            ...
            const isCompleted = (status) => ["completed", "closed"].includes(status.toLowerCase());
            const isProgress = (status) => ["in_progress", "review", "testing"].includes(status.toLowerCase());
            const isNotStarted = (status) => ["not_started", "open", "backlog"].includes(status.toLowerCase());
            ...
            const completionRate = globalTotalCount > 0
            ...
            const projectProgressData = activeProjects.map((project) => {
                const projectTickets = allTickets.filter((t) => t.projectId === project.id);
