...
async function createBankPayrollDetails(req, employeeId, tx = database_1.prisma) {
    try {
        const { bank } = req.body;
...
async function getBankPayrollDetails(req, employeeId) {
    try {
        ...
        const bankDetails = await database_1.prisma.employeeBankDetail.findFirst({
            where: { employeeId },
        });
        const payrollDetails = await database_1.prisma.employeePayrollDetail.findFirst({
            where: { employeeId },
        });
        return {
...
async function updateBankPayrollDetails(req, employeeId, tx = database_1.prisma) {
    try {
        if (!req.user?.id || !req.tenantId)
            throw new Error("Unauthorized");
        const { bank } = req.body;
...
async function deleteBankPayrollDetails(req, employeeId) {
    try {
        ...
        // Delete bank details
        await database_1.prisma.employeeBankDetail.deleteMany({
            where: { employeeId },
        });
        // Delete payroll details
        await database_1.prisma.employeePayrollDetail.deleteMany({
            where: { employeeId },
        });
        return {
            success: true,
