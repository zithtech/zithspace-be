...
async function generateClientCode(tenantId, idPrefix = "CL-") {
  return await database_1.tenantAwarePrisma.withTenant(
    tenantId,
    async (client) => {
...
class ClientV2Controller {
  ...
  static async getClients(req, res) {
    try {
      ...
      const [clients, total] = await Promise.all([
        database_1.tenantAwarePrisma.withTenant(
          req.tenantId,
          async (client) => {
  ...
  static async getClientById(req, res) {
    try {
      ...
      const { id } = req.params;
      const client = await database_1.tenantAwarePrisma.withTenant(
        req.tenantId,
        async (prisma) => {
  ...
  static async updateClient(req, res) {
    try {
      ...
      res
        .status(500)
        .json({ success: false, error: "Failed to update client" });
    }
  }
  // ==============================================
  // CONTACTS
  ...
  static async addDocument(req, res) {
    try {
      ...
      if (!base64 || !fileName || !category || !documentType) {
        res
          .status(400)
          .json({ success: false, error: "Missing required document fields" });
        return;
      }
      // Upload to Cloudflare R2
      const fileUrl = await (0, r2Client_1.uploadClientDocumentToR2)(
        base64,
        fileName,
        req.tenantId,
        clientId,
    ...
    try {
      ...
      await database_1.tenantAwarePrisma.withTenant(
        ...
        async (prisma) => {
          ...
          if (document.fileUrl) {
            try {
              await (0, r2Client_1.deleteFileFromR2)(
                document.fileUrl,
                req.tenantId,
              );
            } catch (r2Error) {
              console.error(
                "Failed to delete file from R2, but continuing with DB deletion:",
                r2Error,
              );
            }
          }
  ...
  static async getProjects(req, res) {
    try {
      ...
      const { clientId } = req.params;
      const projects = await database_1.tenantAwarePrisma.withTenant(
        req.tenantId,
        async (prisma) => {
      ...
      res
        .status(500)
        .json({ success: false, error: "Failed to fetch client projects" });
    }
  }
  static async addProject(req, res) {
    try {
        ...
        async (prisma) => {
          ...
          if (employee) {
            ...
            if (user) {
              ...
              console.log("actualProjectManagerId:", actualProjectManagerId);
            }
          }
      ...
      if (error.code === "P2002") {
        res
          .status(400)
          .json({ success: false, error: "Project code must be unique" });
        return;
      }
      res
        .status(500)
        .json({ success: false, error: "Failed to create and map project" });
    }
  }
  /**
  ...
  static async addAllocation(req, res) {
    try {
      ...
      res
        .status(500)
        .json({ success: false, error: "Failed to add allocation" });
    }
  }
  static async updateAllocation(req, res) {
    try {
      ...
      res
        .status(500)
        .json({ success: false, error: "Failed to update allocation" });
    }
  }
  // ==============================================
  ...
  static async getEmployeesForSelect(req, res) {
    try {
      ...
      const employees = await database_1.tenantAwarePrisma.withTenant(
        req.tenantId,
        async (prisma) => {
