...
class ReimbursementController {
    ...
    static async update(req, res) {
        try {
            ...
            let totalAmount = existing.totalAmount;
            if (itemsArray.length > 0) {
                totalAmount = itemsArray.reduce((sum, item) => sum + Number(item.amount || 0), 0);
            }
            // 5. Perform update in transaction
            const result = await database_1.prisma.$transaction(async (tx) => {
                // Update reimbursement header
                const updated = await tx.reimbursement.update({
                    ...
                    data: {
                        ...
                        submittedAt: status === "SUBMITTED" && !existing.submittedAt
                            ? new Date()
                            : existing.submittedAt,
                        submittedBy: status === "SUBMITTED" && !existing.submittedBy
                            ? req.user?.id
                            : existing.submittedBy,
                    },
                });
    ...
    static async getApprovalList(req, res) {
        try {
            ...
            const subordinates = await database_1.prisma.employeeProjectMapping.findMany({
                where: {
            ...
            const subordinateUsers = await database_1.prisma.user.findMany({
                where: {
                    employeeId: {
            ...
            const reimbursements = await database_1.prisma.reimbursement.findMany({
                where: {
                    tenantId,
                    OR: [
            ...
            // Debug log
            console.log(`📤 [${new Date().toISOString()}] Sending approval data`, formattedData.flatMap(d => d.items.map(i => ({
                itemId: i.id,
    ...
    ===================================================== */
    static async getUserReimbursementLimits(req, res) {
        try {
            if (!req.user || !req.tenantId) {
                res.status(401).json({
                    success: false,
            ...
            // Get policy rules
            const policyRules = await database_1.prisma.reimbursementPolicyRule.findMany({
                where: {
            ...
            // Remove duplicates
            const uniqueResults = Array.from(result.reduce((map, item) => {
    ...
    //   });
    // }
    static async updateReimbursementStatus(reimbursementId, userId) {
        const items = await database_1.prisma.reimbursementItem.findMany({
            where: { reimbursementId },
            select: { status: true }
        });
        ...
        if (newStatus) {
            await database_1.prisma.reimbursement.update({
                where: { id: reimbursementId },
                data: {
                    status: newStatus,
                    updatedById: userId
    ...
    static async approve(req, res) {
        try {
            ...
            let isManager = false;
            if (item.reimbursement.createdBy.employee) {
                const managerCheck = await database_1.prisma.employeeProjectMapping.findFirst({
                    where: {
                        employeeId: item.reimbursement.createdBy.employee.id,
                        reportingManager: userId
                    }
            ...
            let approval;
            if (isManager) {
                approval = await database_1.prisma.reimbursementItemApprover.findFirst({
                    where: {
                        reimbursementItemId,
                        status: "PENDING"
                    }
            ...
            if (!isManager) {
                const previousPending = await database_1.prisma.reimbursementItemApprover.findFirst({
                    where: {
                        reimbursementItemId,
                        level: { lt: approval.level },
                        status: { not: "APPROVED" }
            ...
            const updatedApprover = await database_1.prisma.reimbursementItemApprover.update({
                where: { id: approval.id },
                data: {
                    status: "APPROVED",
                    actedAt: new Date()
            ...
            const totalApprovers = await database_1.prisma.reimbursementItemApprover.count({
                where: { reimbursementItemId }
            });
            // 7️⃣ Count remaining pending
            const remaining = await database_1.prisma.reimbursementItemApprover.count({
                where: {
                    reimbursementItemId,
                    status: "PENDING"
                }
            ...
            if (totalApprovers > 1) {
                ...
                const updatedItem = await database_1.prisma.reimbursementItem.update({
                    where: { id: reimbursementItemId },
                    data: { status: "APPROVED" }
                });
            ...
            await ReimbursementController.updateReimbursementStatus(item.reimbursementId, userId);
            // 🔟 Verification
            const verifyApprover = await database_1.prisma.reimbursementItemApprover.findUnique({
                where: { id: approval.id }
            });
    ...
    static async reject(req, res) {
        try {
            ...
            await ReimbursementController.updateReimbursementStatus(item.reimbursementId, userId);
            // 8️⃣ Verify
            const verify = await database_1.prisma.reimbursementItemApprover.findUnique({
                where: { id: approval.id }
            });
    ...
    static async markAsPaid(req, res) {
        try {
            ...
            await database_1.prisma.reimbursementItemApprover.updateMany({
                where: {
            ...
            await database_1.prisma.reimbursementItem.update({
                where: { id },
                data: { status: "PAID" },
            });
            // 5️⃣ Update parent reimbursement if all items are PAID
            const remaining = await database_1.prisma.reimbursementItem.count({
                where: {
                    reimbursementId: item.reimbursementId,
                    status: { not: "PAID" },
                },
            ...
            //   });
            // }
            await ReimbursementController.updateReimbursementStatus(item.reimbursementId, req.user.id);
            return res.json({
                success: true,
    ...
    static async getFinanceItems(req, res) {
        try {
            if (!req.user) {
                throw new Error("User not authenticated");
            }
...
exports.default = ReimbursementController;
