        ...
        const result = await database_1.prisma.$transaction(async (tx) => {
            ...
            await tx.leaveLedger.create({
                data: {
                    ...
                    employeeId,
                    leaveTypeId,
                    transactionType: adjustmentType === "Credit"
                        ? "adjustment_credit"
                        : "leave_debit",
...
const getLeaveAdjustments = async (req, res) => {
    try {
        const tenantId = req.tenantId;
        const data = await database_1.prisma.leaveAdjustment.findMany({
            where: { tenantId },
            include: {
                employee: {
                    select: {
...
const updateLeaveAdjustment = async (req, res) => {
    try {
        ...
        const numericAmount = parseFloat(amount);
        const existing = await database_1.prisma.leaveAdjustment.findFirst({
            where: { id, tenantId },
        });
        if (!existing) {
        ...
        const result = await database_1.prisma.$transaction(async (tx) => {
            ...
            const previousBalance = lastLedger
                ? new client_1.Prisma.Decimal(lastLedger.balanceAfter)
                : new client_1.Prisma.Decimal(0);
            const oldAmount = new client_1.Prisma.Decimal(existing.amount);
            const newAmount = new client_1.Prisma.Decimal(numericAmount);
...
const deleteLeaveAdjustment = async (req, res) => {
    try {
        ...
        const { id } = req.params;
        const adjustment = await database_1.prisma.leaveAdjustment.findFirst({
            where: { id, tenantId },
        });
        if (!adjustment) {
            return res.status(404).json({
