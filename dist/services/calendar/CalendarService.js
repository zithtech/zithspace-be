...
async function getRedisClient() {
    if (!redisClient) {
        ...
        redisClient.on('error', (err) => logger_1.syncLogger.error('Redis error', { error: err.message }));
 ...
 */
     ...
     */
    static async getEvents(userId, tenantId, startDate, endDate) {
        ...
        const baseEvents = await database_1.prisma.calendarEvent.findMany({
        ...
        for (const event of baseEvents) {
            if (event.isRecurring && !event.externalId.includes('_occ_') && !event.externalId.includes('_RID')) {
                    ...
                    const realRecord = baseEvents.find(be => {
                        ...
                        if (!isSameMaster && be.rrule) {
                            try {
                                ...
                                if (parsed.seriesMasterId === event.externalId)
                                    isSameMaster = true;
                            }
                            catch (e) { }
                        }
                ...
                if (event.exdate && typeof event.exdate === 'string') {
                    ...
                    const excludedDates = event.exdate.split(/[;,]/).map((d) => d.trim().split("T")[0].replace(/-/g, ""));
                ...
                if (masterException) {
                    if (masterException.isCancelled && !masterException.newStartTime) {
                        ...
                        allEvents.push({
                            ...
                            attendees: mex.overrideAttendees || event.attendees,
                        });
                    }
                }
     ...
     */
    static async getValidAccessToken(userId, provider, tenantId) {
    ...
    static async syncEvents(userId, tenantId, provider, startDate, endDate) {
        ...
        const externalIds = externalEvents.map((e) => `${e.externalId}_${userId}`).filter((id) => id != null);
        ...
        await database_1.prisma.calendarEvent.deleteMany({
            where: {
                userId,
                tenantId,
                provider: { in: [client_1.CalendarProvider.GOOGLE, client_1.CalendarProvider.MICROSOFT, client_1.CalendarProvider.ZOHO] },
                OR: [
                    {
                        AND: [
        ...
        for (const event of externalEvents) {
            ...
            else if (provider === client_1.CalendarProvider.MICROSOFT) {
            ...
            else if (provider === client_1.CalendarProvider.ZOHO) {
            ...
            if (isCancelled || isModifiedInstance) {
                ...
                const master = await database_1.prisma.calendarEvent.findFirst({
                ...
                if (originalDateStr) {
                    ...
                    if (isCancelled) {
                        await database_1.prisma.calendarEventException.upsert({
                            where: {
                ...
                if (provider === client_1.CalendarProvider.MICROSOFT && (finalTitle === null || finalTitle === undefined)) {
    ...
    static async createEvent(userId, tenantId, provider, eventData) {
        ...
        if (provider === "MICROSOFT") {
            try {
                const calResponse = await axios_1.default.get("https://graph.microsoft.com/v1.0/me/calendar", {
    ...
    static expandRecurringEvent(master, startLimit, endLimit) {
        const occurrences = [];
        ...
        if (trueRrule && typeof trueRrule === 'string') {
            if (trueRrule.includes("BYDAY=")) {
                ...
                try {
                    ...
                    byDay = days.map((d) => msDayToCode[d.toLowerCase()]).filter((d) => !!d);
                }
                catch (e) {
    ...
    static async updateEvent(userId, tenantId, provider, externalId, eventData, action, occurrenceDate, userEmail) {
        ...
        const isForked = externalId.includes('_RID') || (provider === client_1.CalendarProvider.GOOGLE && externalId.includes('_'));
        const localEvent = await database_1.prisma.calendarEvent.findUnique({
        ...
        if (provider === client_1.CalendarProvider.MICROSOFT && eventData.generateMeeting === undefined) {
        ...
        if (action === 0) {
            ...
            result = await database_1.prisma.calendarEvent.upsert({
                where: { provider_externalId_tenantId: { provider, externalId: mapped.externalId, tenantId } },
                update: mapped,
                create: mapped,
            });
            if (action !== 0 && targetExternalId !== mapped.externalId) {
                try {
                    await database_1.prisma.calendarEvent.delete({
     ...
     */
    static async deleteEvent(userId, tenantId, provider, externalId, action, occurrenceDate, userEmail) {
        const { accessToken, calendarId } = await this.getValidAccessToken(userId, provider, tenantId);
        if (!calendarId)
            throw new Error(`Primary calendar ID not found for ${provider}`);
        const providerImpl = CalendarProviderFactory_1.CalendarProviderFactory.getProvider(provider);
        ...
        if (action === 0 && occurrenceDate && occurrenceDate !== null && occurrenceDate !== undefined) {
            ...
            const masterRecord = await database_1.prisma.calendarEvent.findUnique({
                where: { provider_externalId_tenantId: { provider, externalId: masterExternalId, tenantId } }
            });
            if (masterRecord) {
                const originalDate = new Date(occurrenceDate);
        ...
        if (!action || action === undefined || action === null) {
            ...
            const eventToDelete = await database_1.prisma.calendarEvent.findFirst({
                where: {
                    userId,
                    tenantId,
                    provider,
        ...
        if (action === 2) {
            ...
            const masterEvent = await database_1.prisma.calendarEvent.findFirst({
                where: {
                    userId,
                    tenantId,
                    provider,
    ...
    static resolveMasterExternalId(id) {
        if (!id)
            return "";
        let externalId = id;
     ...
     */
    static mapToCalendarEvent(externalEvent, provider, userId, tenantId, masterRrule) {
        if (provider === client_1.CalendarProvider.ZOHO) {
            ...
            return {
                ...
                attendees: externalEvent.attendees?.map((a) => a.email || a) || null,
            ...
            if (!meetingLink && body) {
                ...
                if (!teamsBusinessMatch) {
                    ...
                    if (backupTeamsBusiness)
                        meetingLink = backupTeamsBusiness[0];
                }
                else {
            ...
            return {
                ...
                rrule: rruleStr,
                meetingLink,
                attendees: externalEvent.attendees?.map((a) => a.emailAddress?.address || a) || null,
    ...
    static parseMicrosoftDate(dateObj) {
    ...
    static microsoftRecurrenceToRRule(recurrence) {
        ...
        if (type === 'weekly' && pattern.daysOfWeek) {
            const days = pattern.daysOfWeek.map((d) => dayMap[d.toLowerCase()]).filter((d) => !!d);
            if (days.length > 0) {
            ...
            const days = pattern.daysOfWeek.map((d) => dayMap[d.toLowerCase()]).filter((d) => !!d);
            const indexMap = { 'first': '1', 'second': '2', 'third': '3', 'fourth': '4', 'last': '-1' };
    ...
    static cleanMicrosoftBody(body) {
        if (!body)
            return null;
     ...
     */
    static async processIncrementalSync(integrationId) {
        const integration = await database_1.prisma.calendarIntegration.findUnique({
        ...
        try {
            ...
            logger_1.syncLogger.warn('Redis lock unavailable, falling back to DB isSyncing flag', { integrationId, error: err.message });
            // Fallback: use DB flag if Redis unavailable
            if (integration.isSyncing)
                return;
        }
        if (!lockAcquired && redis !== null) {
            logger_1.syncLogger.info('Sync already running (lock held), skipping', { integrationId });
            return;
        }
        ...
        try {
            logger_1.syncLogger.info('Incremental sync started', { integrationId, provider, userId });
            const { accessToken, calendarId } = await this.getValidAccessToken(integration.userId, integration.provider, integration.tenantId);
            const providerImpl = CalendarProviderFactory_1.CalendarProviderFactory.getProvider(integration.provider);
            let token = "";
            if (integration.provider === client_1.CalendarProvider.GOOGLE)
                token = integration.googleSyncToken || "";
            else if (integration.provider === client_1.CalendarProvider.MICROSOFT)
                token = integration.microsoftDeltaLink || "";
            else if (integration.provider === client_1.CalendarProvider.ZOHO)
                token = integration.zohoLastSync ? integration.zohoLastSync.toISOString() : "";
            let hasMore = true;
            ...
            if (integration.provider === client_1.CalendarProvider.GOOGLE)
                updateData.googleSyncToken = currentToken;
            else if (integration.provider === client_1.CalendarProvider.MICROSOFT)
                updateData.microsoftDeltaLink = currentToken;
            else if (integration.provider === client_1.CalendarProvider.ZOHO)
                updateData.zohoLastSync = new Date(currentToken);
            await database_1.prisma.calendarIntegration.update({
                where: { id: integrationId },
                data: updateData
            });
            logger_1.syncLogger.info('Incremental sync completed', {
            ...
            logger_1.syncLogger.error('Incremental sync failed', { integrationId, provider, userId, error: error.message });
            const errorCount = (integration.syncErrorCount || 0) + 1;
    ...
    static async handleSyncResult(userId, tenantId, provider, events) {
        logger_1.syncLogger.info(`Handling sync result for ${events.length} events from ${provider}`);
        for (const rawEvent of events) {
            // STEP 3 SaaS Rule: Ignore occurrences and exceptions explicitly for Microsoft
            if (provider === client_1.CalendarProvider.MICROSOFT) {
                if (rawEvent.type === 'occurrence' || rawEvent.type === 'exception') {
                    logger_1.syncLogger.debug(`Ignoring MS ${rawEvent.type} event per SaaS rules`, { id: rawEvent.id });
            ...
            if (provider === client_1.CalendarProvider.ZOHO) {
                ...
                logger_1.syncLogger.debug(`Zoho mapped event`, {
                    uid: rawEvent.uid,
                ...
                logger_1.syncLogger.debug(`Microsoft raw event`, {
                    id: rawEvent.id,
            ...
            if (mapped.isDeleted) {
                ...
                if (provider === client_1.CalendarProvider.GOOGLE && rawEvent.recurringEventId) {
                    await this.handleInstanceCancellation(userId, tenantId, provider, rawEvent);
                }
                else if (provider === client_1.CalendarProvider.ZOHO && (rawEvent.recurrence_id || rawEvent.recurrenceid)) {
                    await this.handleInstanceCancellation(userId, tenantId, provider, rawEvent);
                }
            }
            else {
                ...
                if (provider === client_1.CalendarProvider.GOOGLE)
                    isInstance = !!rawEvent.recurringEventId;
                else if (provider === client_1.CalendarProvider.MICROSOFT)
                    isInstance = rawEvent.type === 'occurrence' || rawEvent.type === 'exception';
                else if (provider === client_1.CalendarProvider.ZOHO)
                    isInstance = !!(rawEvent.recurrence_id || rawEvent.recurrenceid || rawEvent.parent_uid);
                if (isInstance) {
                    ...
                    if (provider === client_1.CalendarProvider.ZOHO && isRecurring && !finalRrule) {
                    ...
                    if (provider === client_1.CalendarProvider.MICROSOFT && existingEvent) {
    ...
    static async handleInstanceCancellation(userId, tenantId, provider, rawEvent) {
        let masterExternalId = "";
    ...
    static async handleInstanceUpsert(userId, tenantId, provider, rawEvent, mapped) {
        let masterExternalId = "";
        let originalDate = null;
        if (provider === client_1.CalendarProvider.GOOGLE) {
            masterExternalId = rawEvent.recurringEventId;
    ...
    static async findMaster(provider, externalId, tenantId, userId) {
        const dbExternalId = externalId.endsWith(`_${userId}`) ? externalId : `${externalId}_${userId}`;
        return database_1.prisma.calendarEvent.findFirst({
