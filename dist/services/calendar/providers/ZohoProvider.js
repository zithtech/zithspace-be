...
class ZohoProvider {
    ...
    formatMeetingUpdateTime(dateInput) {
    ...
    async getEvent(accessToken, calendarId, externalId) {
        ...
        const listRes = await axios_1.default.get(`${ZOHO_CALENDAR_API}/calendars/${calendarId}/events`, { headers: { Authorization: `Zoho-oauthtoken ${accessToken}` } });
        ...
        const found = events.find((e) => ((e.uid && e.uid.toLowerCase() === externalId.toLowerCase()) ||
            (e.event_id && e.event_id.toLowerCase() === externalId.toLowerCase())) &&
            e.etag) || events.find((e) => (e.uid && e.uid.toLowerCase() === externalId.toLowerCase()) ||
            (e.event_id && e.event_id.toLowerCase() === externalId.toLowerCase()));
        if (!found && events.length > 0) {
            console.warn(`[ZohoProvider] Event ${externalId} not found in list. First few UIDs:`, events.slice(0, 5).map((e) => e.uid || e.event_id).join(", "));
        }
        return found || null;
    }
    async getEventETag(accessToken, calendarId, externalId) {
    ...
    async updateEvent(accessToken, calendarId, externalId, eventData, action, occurrenceDate) {
        ...
        if (!etag)
            throw new Error("Could not retrieve Zoho event ETag for update");
        const payload = { ...this.mapToZohoPayload(eventData, true), etag };
        const existing = resolved.event || {};
        // Define helpers first
        const cleanDate = (d) => typeof d === 'string' ? d.replace(/\+/g, "").replace(/0000$/, "Z") : d;
        const forceZ = (dt) => {
            if (!dt)
                return dt;
        ...
        if (action === 2) {
            ...
            const putRes = await axios_1.default.put(resolved.url, formBody, {
                headers: {
                    Authorization: `Zoho-oauthtoken ${accessToken}`,
                    "Content-Type": "application/x-www-form-urlencoded",
                },
            ...
            if (putRes.status === 500) {
                ...
                if (freshEvent?.etag && freshEvent.etag !== etag) {
                    ...
                    const retryRes = await axios_1.default.put(resolved.url, retryFormBody, {
                        headers: {
                            Authorization: `Zoho-oauthtoken ${accessToken}`,
                            "Content-Type": "application/x-www-form-urlencoded",
                        },
                    ...
                    const altRes = await axios_1.default.put(alternativeUrl, altFormBody, {
                        headers: {
                            Authorization: `Zoho-oauthtoken ${accessToken}`,
                            "Content-Type": "application/x-www-form-urlencoded",
                        },
                    ...
                    const minRes = await axios_1.default.put(resolved.url, minFormBody, {
                        headers: {
                            Authorization: `Zoho-oauthtoken ${accessToken}`,
                            "Content-Type": "application/x-www-form-urlencoded",
                        },
                    ...
                    if (eventData.generateMeeting) {
                        try {
                            ...
                            const createRes = await axios_1.default.post(`${ZOHO_CALENDAR_API}/calendars/${calendarId}/events`, createFormBody, {
                                headers: {
                                    Authorization: `Zoho-oauthtoken ${accessToken}`,
                                    "Content-Type": "application/x-www-form-urlencoded",
                                },
                            ...
                            if (createRes.status < 400)
                                return createRes.data?.events?.[0];
                        }
                        catch (recreateErr) {
            ...
            if (payload.dateandtime)
                payload.dateandtime = forceZ(payload.dateandtime);
            if (payload.title === existing.title)
                delete payload.title;
            if (payload.description === existing.description)
                delete payload.description;
            if (payload.location === existing.location)
        ...
        try {
            const response = await axios_1.default.patch(eventUrl, { eventdata: payload }, {
            ...
            if (error.response?.status === 409) {
                ...
                if (freshEvent?.etag) {
                    payload.etag = freshEvent.etag;
                    const retryRes = await axios_1.default.patch(eventUrl, { eventdata: payload }, {
                        headers: { Authorization: `Zoho-oauthtoken ${accessToken}`, "Content-Type": "application/json" },
                    });
                    return retryRes.data?.events?.[0];
                }
    ...
    async deleteEvent(accessToken, calendarId, externalId, action, occurrenceDate) {
        ...
        if (action !== undefined) {
            ...
            const typeMap = { 0: "only", 1: "following" };
            if (typeMap[action]) {
        ...
        try {
            const response = await axios_1.default.delete(eventUrl, {
                headers: { Authorization: `Zoho-oauthtoken ${accessToken}` },
            });
            ...
            if (error.response?.status === 409) {
                ...
                if (freshEvent?.etag) {
                    ...
                    try {
                        const retryRes = await axios_1.default.delete(retryUrl, {
                            headers: { Authorization: `Zoho-oauthtoken ${accessToken}` },
                        });
    ...
    mapToZohoPayload(data, isUpdate = false) {
        const payload = {
            title: data.title,
            dateandtime: {
        ...
        if (data.isRecurring) {
            if (data.recurringDays && data.recurringDays.length > 0 && data.recurringDays.length < 7) {
                const days = data.recurringDays.map(d => typeof d === 'string' ? d.replace(/[^A-Z]/g, '') : d).filter(d => !!d).join(",");
                payload.rrule = `FREQ=WEEKLY;INTERVAL=1;BYDAY=${days}`;
                if (data.generateMeeting && !isUpdate)
                    payload.rrule += ";COUNT=50";
            }
            else {
                payload.rrule = (data.generateMeeting && !isUpdate) ? "FREQ=DAILY;INTERVAL=1;COUNT=50" : "FREQ=DAILY;INTERVAL=1";
            }
    ...
    async resolveEventUrl(accessToken, calendarId, externalId, eventObj) {
        if (!externalId)
