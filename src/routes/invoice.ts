...
router.get('/deleted', requirePermission(Permissions.INVOICE_TRASH_READ), requireSubscriptionFeature('finance_invoice_invoice_trash', { exact: false }), (req, res) => InvoiceController.getDeletedInvoices(req, res));
...
// Create new invoice
router.post('/', requirePermission(Permissions.INVOICE_CREATE), requireSubscriptionFeature('finance_invoice_newinvoice', { exact: false }), (req, res) => InvoiceController.createInvoice(req, res));
...
router.delete('/:id', requirePermission(Permissions.INVOICE_DELETE), requireSubscriptionFeature('finance_invoice_invoice_trash', { exact: false }), (req, res) => InvoiceController.deleteInvoice(req, res));
...
router.post('/bulk-permanent-delete', requirePermission(Permissions.INVOICE_TRASH_DELETE), requireSubscriptionFeature('finance_invoice_invoice_trash', { exact: false }), (req, res) => InvoiceController.bulkPermanentDeleteInvoices(req, res));

// Update invoice
router.put('/:id', requirePermission(Permissions.INVOICE_UPDATE), requireSubscriptionFeature('finance_invoice_newinvoice', { exact: false }), (req, res) => InvoiceController.updateInvoice(req, res));
