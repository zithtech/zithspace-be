// src/db/attendancePool.ts
//
// Dedicated PostgreSQL connection pool for the Attendance module.
//
// WHY a separate pool (not Prisma):
//   Attendance is now a pure raw-SQL module. It owns its own pg.Pool pointed at
//   the same DATABASE_URL as the rest of the app, so its data access is fully
...
//   pooled connection), and runs every query on that same connection.
...
if (!connectionString) {
  throw new Error('[attendance] DATABASE_URL is not set — cannot create pg pool');
}
...
export const attendancePool = new Pool({
  connectionString,
  ssl: needsSsl ? { rejectUnauthorized: false } : undefined,
  max: Number(process.env.ATT_PG_POOL_MAX ?? 10),
  idleTimeoutMillis: 30_000,
  connectionTimeoutMillis: 10_000,
  application_name: 'attendance',
});

attendancePool.on('error', (err) => {
  // Errors on idle clients must be handled or they crash the process.
  console.error('[attendance] unexpected error on idle pool client:', err);
});
 ...
 * Controllers/repos receive this and must NOT reach for `attendancePool` directly.
 */
...
export async function withTenant<T>(
  ...
  if (!tenantId) {
    throw new Error('[attendance] withTenant requires a non-empty tenantId');
  }

  const client: PoolClient = await attendancePool.connect();
  try {
...
export async function closeAttendancePool(): Promise<void> {
