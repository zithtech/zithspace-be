// src/modules/opening-management/db/pool.ts
//
// Dedicated PostgreSQL connection pool for the Opening Management module.
//
// WHY a separate pool (not Prisma):
//   Opening Management is a pure raw-SQL module. It owns its own pg.Pool pointed
//   at the same DATABASE_URL as the rest of the app, so its data access is fully
//   decoupled from Prisma. The legacy Prisma-managed `opening_managements` table
//   is left untouched — this module owns its own `om_*` tables.
//
...
if (!connectionString) {
  throw new Error('[opening-management] DATABASE_URL is not set — cannot create pg pool');
}
...
export const omPool = new Pool({
  connectionString,
  ssl: needsSsl ? { rejectUnauthorized: false } : undefined,
  max: Number(process.env.OM_PG_POOL_MAX ?? 10),
  idleTimeoutMillis: 30_000,
  connectionTimeoutMillis: 10_000,
  application_name: 'opening-management',
});

omPool.on('error', (err) => {
  // Errors on idle clients must be handled or they crash the process.
  console.error('[opening-management] unexpected error on idle pool client:', err);
});
 ...
 * Repositories receive this and must NOT reach for `omPool` directly.
 */
...
export async function withTenant<T>(
  ...
  if (!tenantId) {
    throw new Error('[opening-management] withTenant requires a non-empty tenantId');
  }

  const client: PoolClient = await omPool.connect();
  try {
...
export async function closeOpeningManagementPool(): Promise<void> {
  await omPool.end();
}
