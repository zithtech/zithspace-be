// src/modules/payroll/http.ts
// Small HTTP helpers shared by payroll controllers so each handler stays thin.
// Mirrors src/modules/performance-report/http.ts and leave-v2/http.ts.
...
/**
 * Wrap an async controller: translate thrown PayrollError / ZodError into the
...
export function handle(fn: Handler) {
  return async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      console.error('[payroll] unhandled error:', err);
