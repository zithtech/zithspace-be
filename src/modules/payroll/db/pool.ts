// src/modules/payroll/db/pool.ts
//
// Dedicated PostgreSQL connection pool for the Payroll 2.0 module.
//
// WHY a separate pool (not Prisma):
//   Payroll 2.0 is a pure raw-SQL module (product directive — it replaces the
//   legacy Prisma payroll). It owns its own pg.Pool pointed at the same
//   DATABASE_URL as the rest of the app, fully decoupled from Prisma. Mirrors
//   src/modules/leave-v2 and src/modules/performance-report.
//
...
if (!connectionString) {
  throw new Error('[payroll] DATABASE_URL is not set — cannot create pg pool');
}
...
export const payrollPool = new Pool({
  connectionString,
  ssl: needsSsl ? { rejectUnauthorized: false } : undefined,
  max: Number(process.env.PAY_PG_POOL_MAX ?? 10),
  idleTimeoutMillis: 30_000,
  connectionTimeoutMillis: 10_000,
  application_name: 'payroll',
});

payrollPool.on('error', (err) => {
  // Errors on idle clients must be handled or they crash the process.
  console.error('[payroll] unexpected error on idle pool client:', err);
});
 ...
 * Repositories receive this and must NOT reach for `payrollPool` directly.
 */
...
export async function withTenant<T>(
  ...
  if (!tenantId) {
    throw new Error('[payroll] withTenant requires a non-empty tenantId');
  }

  const client: PoolClient = await payrollPool.connect();
  try {
...
export async function closePayrollPool(): Promise<void> {
