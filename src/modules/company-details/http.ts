// src/modules/company-details/http.ts
// Small HTTP helpers shared by company-details controllers so each handler
// stays thin. Mirrors the opening-management module's conventions.

import { NextFunction, Request, Response } from 'express';
import { ZodError } from 'zod';
import { AuthRequest } from '@/types';
import { Actor, CompanyDetailsError } from './types';
...
export function actorOf(req: AuthRequest): Actor {
  ...
  return { tenantId: req.tenantId as string, userId: u.id as string };
}

type Handler = (req: AuthRequest, res: Response) => Promise<unknown>;

/**
 * Wrap an async controller: translate thrown CompanyDetailsError / ZodError
 * into the platform's `{ success, error, code }` envelope; everything else → 500.
 */
export function handle(fn: Handler) {
  return async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      if (err instanceof CompanyDetailsError) {
      ...
      console.error('[company-details] unhandled error:', err);
