// src/modules/reimbursement-v2/http.ts
// Small HTTP helpers shared by reimbursement-v2 controllers so each handler stays thin.

import { Response } from 'express';
import { ZodError } from 'zod';
import { AuthRequest } from '@/types';
import { Actor, ReimbursementV2Error } from './types';
...
/**
 * Wrap an async controller: translate thrown ReimbursementV2Error / ZodError into
 * the platform's `{ success, error, code }` envelope; everything else → 500.
 */
export function handle(fn: Handler) {
  return async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      if (err instanceof ReimbursementV2Error) {
      ...
      console.error('[reimbursement-v2] unhandled error:', err);
