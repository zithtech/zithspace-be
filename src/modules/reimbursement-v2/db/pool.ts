// src/modules/reimbursement-v2/db/pool.ts
//
// Dedicated PostgreSQL connection pool for the Reimbursement 2.0 module.
//
// WHY a separate pool (not Prisma):
//   Reimbursement 2.0 is a pure raw-SQL module. It owns its own pg.Pool pointed
//   at the same DATABASE_URL as the rest of the app, so its data access is fully
//   decoupled from Prisma — mirroring the Leave 2.0 module.
//
// THE CRITICAL INVARIANT — tenant isolation:
//   The platform enforces multi-tenancy through Postgres RLS, which reads
//   `current_setting('app.current_tenant_id')`. A raw query on a different
//   connection has NO such context, so it would be blocked by RLS or — if RLS is
//   bypassed — leak every tenant's rows.
//
...
if (!connectionString) {
  throw new Error('[reimbursement-v2] DATABASE_URL is not set — cannot create pg pool');
}
...
export const rb2Pool = new Pool({
  connectionString,
  ssl: needsSsl ? { rejectUnauthorized: false } : undefined,
  max: Number(process.env.RB2_PG_POOL_MAX ?? 10),
  idleTimeoutMillis: 30_000,
  connectionTimeoutMillis: 10_000,
  application_name: 'reimbursement-v2',
});

rb2Pool.on('error', (err) => {
  // Errors on idle clients must be handled or they crash the process.
  console.error('[reimbursement-v2] unexpected error on idle pool client:', err);
});
 ...
 * Repositories receive this and must NOT reach for `rb2Pool` directly.
 */
...
export async function withTenant<T>(
  ...
  if (!tenantId) {
    throw new Error('[reimbursement-v2] withTenant requires a non-empty tenantId');
  }

  const client: PoolClient = await rb2Pool.connect();
  try {
...
export async function closeReimbursementV2Pool(): Promise<void> {
  await rb2Pool.end();
}
