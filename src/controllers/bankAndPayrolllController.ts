...
export async function getAllBankPayrollDetails(req: AuthRequest) {
  try {
    ...
    return await withTenant(req.tenantId, async (db) => {
      ...
      const [bankRes, payrollRes] = await Promise.all([
        db.query(
          `SELECT * FROM employee_bank_details WHERE employee_id = ANY($1::uuid[])`,
          [employeeIds],
        ),
        db.query(
          `SELECT * FROM employee_payroll_details WHERE employee_id = ANY($1::uuid[])`,
          [employeeIds],
        ),
      ]);
      ...
      for (const b of bankRes.rows) {
        if (!bankByEmp.has(b.employee_id)) bankByEmp.set(b.employee_id, b);
      }
...
export async function updateBankPayrollDetails(
  ...
  try {
    ...
    return await withClient(req, client, async (db) => {
      ...
      const existingBankRes = await db.query(
        `SELECT id FROM employee_bank_details WHERE employee_id = $1 LIMIT 1`,
        [employeeId],
      );
      ...
      const existingPayrollRes = await db.query(
        `SELECT id FROM employee_payroll_details WHERE employee_id = $1 LIMIT 1`,
        [employeeId],
      );
...
export async function deleteBankPayrollDetails(
  ...
  try {
    ...
    return await withTenant(req.tenantId, async (db) => {
      ...
      // Delete bank details
      await db.query(
        `DELETE FROM employee_bank_details WHERE employee_id = $1`,
        [employeeId],
      );

      // Delete payroll details
      await db.query(
        `DELETE FROM employee_payroll_details WHERE employee_id = $1`,
        [employeeId],
      );
