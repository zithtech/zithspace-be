...
export class ShiftsController {
   ...
   */
  static async getShifts(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      if (search) {
        where.OR = [
          ...
          { startTime: { contains: search as string, mode: 'insensitive' } },
          { endTime: { contains: search as string, mode: 'insensitive' } }
        ];
      }
   ...
   */
  static async getShiftById(req: AuthRequest, res: Response): Promise<void> {
    try {
      if (!req.tenantId || !req.user) {
        res.status(400).json({
          success: false,
      ...
      const shift = await tenantAwarePrisma.withTenant(req.tenantId, async (client) => {
   ...
   */
  static async createShift(req: AuthRequest, res: Response): Promise<void> {
    try {
      if (!req.tenantId || !req.user) {
        res.status(400).json({
          success: false,
      ...
      await tenantAwarePrisma.withTenant(req.tenantId, async (client) => {
        ...
        if (existingShift) {
          throw new ValidationError('Shift with this name already exists in this tenant');
        }
   ...
   */
  static async updateShift(req: AuthRequest, res: Response): Promise<void> {
    try {
      if (!req.tenantId || !req.user) {
        res.status(400).json({
          success: false,
      ...
      await tenantAwarePrisma.withTenant(req.tenantId, async (client) => {
        ...
        if (!existingShift) {
          throw new NotFoundError('Shift not found in this tenant');
        }
   ...
   */
  static async deleteShift(req: AuthRequest, res: Response): Promise<void> {
    try {
      if (!req.tenantId || !req.user) {
        res.status(400).json({
          success: false,
      ...
      await tenantAwarePrisma.withTenant(req.tenantId, async (client) => {
        ...
        if (usersWithShift > 0) {
          throw new ValidationError(`Cannot delete shift. ${usersWithShift} users are currently assigned to this shift.`);
        }
   ...
   */
  static async activateShift(req: AuthRequest, res: Response): Promise<void> {
    try {
      if (!req.tenantId || !req.user) {
        res.status(400).json({
          success: false,
   ...
   */
  static async assignShiftToUser(req: AuthRequest, res: Response): Promise<void> {
    try {
      if (!req.tenantId || !req.user) {
        res.status(400).json({
          success: false,
      ...
      await tenantAwarePrisma.withTenant(req.tenantId, async (client) => {
        ...
        if (!shift) {
          throw new NotFoundError('Active shift not found in this tenant');
        }
        ...
        if (!user) {
          throw new NotFoundError('Active user not found in this tenant');
        }
   ...
   */
  static async removeShiftFromUser(req: AuthRequest, res: Response): Promise<void> {
    try {
      if (!req.tenantId || !req.user) {
        res.status(400).json({
          success: false,
   ...
   */
  static async getShiftsForSelect(req: AuthRequest, res: Response): Promise<void> {
    try {
      if (!req.tenantId || !req.user) {
        res.status(400).json({
          success: false,
   ...
   */
  static async getUsersByShift(req: AuthRequest, res: Response): Promise<void> {
    try {
      if (!req.tenantId || !req.user) {
        res.status(400).json({
          success: false,
