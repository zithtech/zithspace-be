  ...
  try {
    ...
    return await withClient(req, client, async (db) => {
      ...
      if (employment.positionId) {
        ...
        await db.query(
          `INSERT INTO employee_role_history (tenant_id, employee_id, role_name, start_date) VALUES ($1, $2, $3, $4)`,
          [req.tenantId, employeeId, roleName, joiningDate]
        );
      }
  ...
  try {
    ...
    return await withClient(req, client, async (db) => {
      ...
      const existingWorkRes = await db.query(
        `SELECT id, position_id FROM employee_work_details
          WHERE employee_id = $1
          ORDER BY created_at ASC
          LIMIT 1`,
        [employeeId],
      ...
      if (existingWorkDetails) {
        ...
        if (employment.positionId) {
          ...
          await db.query(
            `INSERT INTO employee_role_history (tenant_id, employee_id, role_name, start_date) VALUES ($1, $2, $3, $4)`,
            [tenantId, employeeId, roleName, joiningDate]
          );
        }
      }
