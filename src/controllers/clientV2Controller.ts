...
export class ClientV2Controller {
        ...
        try {
            ...
            const contactsRes = await pool.query(
                `SELECT cc.*, u.name as created_by_name, u.avatar_url as created_by_avatar_url
                 ...
                 WHERE cc.client_id = $1 AND cc.tenant_id = $2`,
                [id, req.tenantId]
            );
    ...
    static async addContact(req: AuthRequest, res: Response): Promise<void> {
        try {
            if (!req.tenantId || !req.user) {
                res.status(400).json({ success: false, error: 'Tenant and user context required' } as ApiResponse);
                return;
            }
            ...
            if (data.firstName && typeof data.firstName === 'string' && data.firstName.trim().length > 50) {
                res.status(400).json({ success: false, error: 'First name cannot exceed 50 characters' } as ApiResponse);
                return;
            }
            if (data.lastName && typeof data.lastName === 'string' && data.lastName.trim().length > 50) {
                res.status(400).json({ success: false, error: 'Last name cannot exceed 50 characters' } as ApiResponse);
                return;
            }
            ...
            const fullContactRes = await pool.query(
                `SELECT cc.*, u.name as created_by_name, u.avatar_url as created_by_avatar_url
                 FROM client_contacts_v2 cc
                 LEFT JOIN users u ON u.id = cc.created_by_id
    ...
    static async updateContact(req: AuthRequest, res: Response): Promise<void> {
        try {
            ...
            if ('firstName' in data && (data as any).firstName && typeof (data as any).firstName === 'string' && (data as any).firstName.trim().length > 50) {
                res.status(400).json({ success: false, error: 'First name cannot exceed 50 characters' } as ApiResponse);
                return;
            }
            if ('lastName' in data && (data as any).lastName && typeof (data as any).lastName === 'string' && (data as any).lastName.trim().length > 50) {
                res.status(400).json({ success: false, error: 'Last name cannot exceed 50 characters' } as ApiResponse);
                return;
            }
    ...
    static async getProjects(req: AuthRequest, res: Response): Promise<void> {
        try {
            ...
            const params: any[] = [clientId, req.tenantId];
            let pIdx = 3;
    ...
    static async getContacts(req: AuthRequest, res: Response): Promise<void> {
        try {
            ...
            const countRes = await pool.query(
                `SELECT COUNT(*)::int AS total FROM client_contacts_v2 cc WHERE ${whereClause}`,
                params
            );
            const total = countRes.rows[0]?.total || 0;
            ...
            res.status(500).json({ success: false, error: 'Failed to fetch contacts' } as ApiResponse);
        }
    }
    ...
    static async getDocuments(req: AuthRequest, res: Response): Promise<void> {
        try {
            ...
            const countRes = await pool.query(
                `SELECT COUNT(*)::int AS total FROM client_documents_v2 cd WHERE ${whereClause}`,
                params
            );
            const total = countRes.rows[0]?.total || 0;
            ...
            if (documents.length > 0) {
                ...
                if (uploaderIds.length > 0) {
                    ...
                    const uploaderMap = new Map(uploaderRes.rows.map((u: any) => [u.id, u]));
                    documents.forEach((d) => {
                        if (d.uploadedById && uploaderMap.has(d.uploadedById)) {
                            const u: any = uploaderMap.get(d.uploadedById);
    ...
    static async getClientInvoices(req: AuthRequest, res: Response): Promise<void> {
        try {
            ...
            const params: any[] = [req.tenantId, customerIds, validStatuses];
            let pIdx = 4;
