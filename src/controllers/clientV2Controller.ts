...
export class ClientV2Controller {
  ...
  static async getClientById(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      const { id } = req.params;
      const client = await tenantAwarePrisma.withTenant(
        req.tenantId,
        async (prisma) => {
      ...
      if (!client) {
        res
          .status(404)
          .json({ success: false, error: "Client not found" } as ApiResponse);
        return;
      }
  ...
  static async getProjects(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      const projects = await tenantAwarePrisma.withTenant(
        req.tenantId,
        async (prisma) => {
  ...
  static async updateProject(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      await tenantAwarePrisma.withTenant(req.tenantId, async (prisma) => {
        ...
        if (currency !== undefined) mappingUpdateData.budgetType = currency;
  ...
  static async addAllocation(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      await tenantAwarePrisma.withTenant(req.tenantId, async (prisma) => {
        ...
        res
          .status(201)
          .json({ success: true, data: allocation } as ApiResponse);
      });
    } catch (error) {
      res.status(500).json({
        success: false,
  ...
  static async getEmployeesForSelect(
    req: AuthRequest,
    res: Response,
  ): Promise<void> {
    try {
      ...
      const employees = await tenantAwarePrisma.withTenant(
        req.tenantId,
        async (prisma) => {
