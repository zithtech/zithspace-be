 ...
 */
function parseZohoDate(raw: string): Date {
 ...
 */
function toZohoDate(iso: string | Date, allDay: boolean): string {
...
export class CalendarController {
    ...
    static async connect(req: AuthRequest, res: Response): Promise<void> {
        try {
            ...
            (req as any).session.zohoUserId = req.user.id;
            (req as any).session.zohoState = req.user.id; // simple state = userId
    ...
    static async callback(req: AuthRequest, res: Response): Promise<void> {
        try {
            ...
            if (oauthError) {
                ...
                return res.redirect(`${FRONTEND_URL}/calendar?error=zoho_denied`) as any;
            }
            ...
            if (!access_token) {
                ...
                return res.redirect(`${FRONTEND_URL}/calendar?error=token_exchange_failed`) as any;
            }
    ...
    static async getEvents(req: AuthRequest, res: Response): Promise<void> {
        try {
            ...
            const zohoRes = await axios.get(
                `${ZOHO_CALENDAR_API}/calendars/${user.zohoCalendarId}/events`,
                { headers: { Authorization: `Zoho-oauthtoken ${accessToken}` } }
            );

            const zohoEvents: any[] = zohoRes.data?.events || [];
            ...
            const validEvents = zohoEvents.filter((e: any) => e.uid || e.event_id);
            ...
            validEvents.forEach((e: any) => {
                ...
                if (!groupedEvents.has(uid)) groupedEvents.set(uid, []);
            ...
            for (const [uid, items] of groupedEvents.entries()) {
                ...
                const master = items.find((it: any) => it.estatus !== "deleted") || items[0];
                const deletedInstances = items.filter((it: any) => it.estatus === "deleted");
                ...
                deletedInstances.forEach((d: any) => {
                ...
                const existing = await prisma.zohoEvent.findUnique({
                    where: { id: deterministicId },
                    select: { exdate: true }
                });
                const localExdates: string[] = Array.isArray(existing?.exdate)
                    ? (existing?.exdate as string[])
                    : (typeof existing?.exdate === "string" ? (existing?.exdate as string).split(",") : []);

                // Merge: Zoho's new findings + our local manual record
                const mergedExdates = Array.from(new Set([...exdates, ...localExdates]));

                upsertOps.push(
                    prisma.zohoEvent.upsert({
                        where: { id: deterministicId },
                        create: {
                            id: deterministicId,
                            eventId: uid,
                            ...
                            meetingLink: master.conference_data?.meetingdata?.meeting_link || master.app_data?.meetingdata?.meetinglink || (master as any).meetingLink || null,
                        },
                        update: {
                            title: master.title || "Untitled",
                            description: master.description || null,
            ...
            const dbEvents = await prisma.zohoEvent.findMany({
                where,
    ...
    static async createEvent(req: AuthRequest, res: Response): Promise<void> {
        try {
            ...
            const dbEvent = await prisma.zohoEvent.upsert({
                where: { id: deterministicId },
                create: {
                    id: deterministicId,
                    userId: req.user.id,
    ...
    static async updateEvent(req: AuthRequest, res: Response): Promise<void> {
        try {
            ...
            const dbEvent = await prisma.zohoEvent.findFirst({
            ...
            if (!etag) {
                ...
                res.status(500).json({ success: false, error: "Could not retrieve event ETag from Zoho" } as ApiResponse);
                return;
            }
            ...
            const eventPayload: any = {
                ...
                isallday: isAllDay === undefined ? (dbEvent as any).isAllDay : !!isAllDay,
                isrep: isRecurring !== undefined ? !!isRecurring : (dbEvent as any).isRecurring,
            };

            if (!(isAllDay === undefined ? (dbEvent as any).isAllDay : !!isAllDay)) {
                eventPayload.dateandtime.timezone = "Asia/Kolkata";
            }

            const isActuallyMeeting = generateMeeting || !!(dbEvent as any).meetingLink;
            if (isRecurring !== undefined ? isRecurring : (dbEvent as any).isRecurring) {
                // Zoho requires a limit for recurring meetings
            ...
            const zohoRes = await axios.put(eventUrl, formBody, {
                headers: {
                    Authorization: `Zoho-oauthtoken ${accessToken}`,
                    "Content-Type": "application/x-www-form-urlencoded",
                },
            ...
            const dataUpdate: any = {
                ...
                location: location ?? dbEvent.location,
                isAllDay: isAllDay !== undefined ? !!isAllDay : (dbEvent as any).isAllDay,
                isRecurring: isRecurring !== undefined ? !!isRecurring : (dbEvent as any).isRecurring,
                rrule: (isRecurring !== undefined ? isRecurring : (dbEvent as any).isRecurring)
                    ? (isActuallyMeeting ? "FREQ=DAILY;INTERVAL=1;COUNT=50" : "FREQ=DAILY;INTERVAL=1")
                    : null,
                exdate: updatedZohoEvent?.exdate || (dbEvent as any).exdate || null,
                calendar: calendar !== undefined ? calendar : (dbEvent as any).calendar,
                sourceType: sourceType !== undefined ? sourceType : (dbEvent as any).sourceType,
                attendees: attendees || (dbEvent as any).attendees || [],
                meetingLink: meetingLink,
                updatedById: req.user.id,
                tenantId: req.user.tenantId!,
            };

            const updatedEvent = await prisma.zohoEvent.update({
                where: { id },
                data: dataUpdate,
            });
    ...
    static async deleteEvent(req: AuthRequest, res: Response): Promise<void> {
        try {
            ...
            const zohoDeleteRes = await axios.delete(deleteUrl, {
                headers: {
                    Authorization: `Zoho-oauthtoken ${accessToken}`,
                    "Content-Type": "application/x-www-form-urlencoded",
                },
            ...
            if (shouldDeleteLocally) {
                await prisma.zohoEvent.delete({ where: { id } });
                ...
                if (dbEvent.exdate) {
                    currentExdates = Array.isArray(dbEvent.exdate) ? dbEvent.exdate as string[] : (typeof dbEvent.exdate === "string" ? dbEvent.exdate.split(",") : []);
                }

                if (!currentExdates.includes(formattedExdate)) {
                    currentExdates.push(formattedExdate);
                    await prisma.zohoEvent.update({
                        where: { id },
            ...
            const isNotFound = Array.isArray(zohoError?.error) && zohoError?.error?.some((e: any) => e.message === "EVENT_NOTFOUND");

            if (isNotFound) {
                console.log("[Zoho] Event already deleted on Zoho, cleaning up local DB...");
                const { id } = req.params;
                const dbEvent = await prisma.zohoEvent.findUnique({ where: { id } });
                const { action } = req.query as Record<string, string>;

                if (dbEvent && (!dbEvent.isRecurring || action === "2")) {
                    await prisma.zohoEvent.delete({ where: { id } }).catch(() => { });
                }
            ...
            res.status(500).json({ success: false, error: "Failed to delete Zoho event" } as ApiResponse);
        }
    }
