...
import { CalendarProvider, MailProvider } from "@prisma/client";
...
export class UnifiedAuthService {
    ...
    static getAuthUrl(provider: CalendarProvider, userId: string): string {
        ...
        switch (provider) {
            ...
            case CalendarProvider.ZOHO:
                ...
                // Zoho scopes are comma-separated or space-separated depending on the prompt
     ...
     */
    static async handleCallback(provider: CalendarProvider, code: string, state: string, userId: string, tenantId: string): Promise<void> {
        ...
        // 2. Create/Update MailAccount FIRST to get an ID (with NULL tokens)
        const mailAccount = await prisma.mailAccount.upsert({
            where: {
                provider_email_tenantId: {
                    provider: provider as unknown as MailProvider,
                    email: email,
                    tenantId: tenantId
                }
            },
