...
export class MicrosoftProvider implements ICalendarProvider {
    ...
    private getCalendarBaseUrl(calendarId: string): string {
    ...
    async getIncrementalChanges(accessToken: string, calendarId: string, token?: string): Promise<IncrementalSyncResult> {
        ...
        try {
            ...
            if (error.response) {
                ...
                if (token) console.error(`[MicrosoftProvider] Token length: ${token.length}`);
            }
    ...
    async createEvent(accessToken: string, calendarId: string, eventData: CalendarEventData): Promise<any> {
        ...
        let userEmail = "";
        try {
            const meRes = await axios.get(`${MS_GRAPH_API}/me`, {
        ...
        try {
            const response = await axios.post(
                `${this.getCalendarBaseUrl(calendarId)}/events`,
                payload,
                {
                    headers: {
                        Authorization: `Bearer ${accessToken}`,
            ...
            if (useConversionWorkaround) {
                ...
                if (!createdEvent.isOnlineMeeting) {
                    ...
                    const patchKick = await axios.patch(
                        `${this.getCalendarBaseUrl(calendarId)}/events/${createdEvent.id}`,
                ...
                const finalPatch = await axios.patch(
                    `${this.getCalendarBaseUrl(calendarId)}/events/${createdEvent.id}`,
    ...
    async updateEvent(accessToken: string, calendarId: string, externalId: string, eventData: CalendarEventData, action?: number, occurrenceDate?: string): Promise<any> {
        ...
        try {
            const response = await axios.patch(
                `${this.getCalendarBaseUrl(calendarId)}/events/${targetId}`,
    ...
    async deleteEvent(accessToken: string, calendarId: string, externalId: string, action?: number, occurrenceDate?: string): Promise<void> {
        try {
            ...
            if (action === 0 && occurrenceDate) {
                try {
                    const eventRes = await axios.get(deleteUrl, {
                        headers: { Authorization: `Bearer ${accessToken}`, "Prefer": 'outlook.timezone="UTC"' }
                    });
                    ...
                    if (event.type === 'seriesMaster') {
                        ...
                        const instancesRes = await axios.get(`${deleteUrl}/instances`, {
                            headers: { Authorization: `Bearer ${accessToken}`, "Prefer": 'outlook.timezone="UTC"' },
                            params: { startDateTime: windowStart.toISOString(), endDateTime: windowEnd.toISOString() }
                        });

                        const instance = (instancesRes.data.value || []).find((inst: any) => {
    ...
    private mapToMicrosoftPayload(data: CalendarEventData, userEmail: string = "", isUpdate: boolean = false, skipRecurrence: boolean = false): any {
        ...
        if (data.attendees && data.attendees.length > 0) {
            payload.attendees = data.attendees.map(attendee => {
                ...
                if (typeof attendee === 'string') {
                    ...
                    emailStr = (attendee as any).email || (attendee as any).emailAddress?.address || (attendee as any).address || attendee;
                    nameStr = (attendee as any).displayName || (attendee as any).name || (attendee as any).emailAddress?.name || emailStr;

                    // If we still found an object for email, try one more level or convert to string
                    if (typeof emailStr !== 'string') {
                        emailStr = (emailStr as any).address || (emailStr as any).email || String(emailStr);
                    }
